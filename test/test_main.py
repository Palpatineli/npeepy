...
def test_mi():
    ...
    for _ in range(1000):
        ...
        res.append(ee.mi(a[:, np.newaxis], b[:, np.newaxis]))
...
def cmutual(size, mean, cov, sample_no=100, ci=(0.025, 0.975)):
    ...
    """shuffle cmi"""
    np.random.seed(12345)
    ent = [ee.cmi(*(x.reshape(-1, 1) for x in mn(mean, cov, size).T)) for _ in range(sample_no)]
    ent = np.sort(ent)
...
def mutual(size, mean, cov, sample_no=100, ci=(0.025, 0.975)):
    # type: (int, np.ndarray, np.ndarray, int, Tuple[float, float]) -> Tuple[float, Tuple[float, float]]
    np.random.seed(12345)
    ent = [ee.mi(*(x.reshape(-1, 1) for x in mn(mean, cov, size).T[0: 2, :])) for _ in range(sample_no)]
    ent = np.sort(ent)
    return np.mean(ent), (ent[int(ci[0] * sample_no)], ent[int(ci[1] * sample_no)])
...
def test_entropyd():
    ...
    y = [0, 1, 0, 1, 0, 1, 0, 1]
    z = [0, 1, 0, 1, 1, 0, 1, 0]
    assert(ee.entropyd(x) == 1)
    assert(ee.entropyd(y) == 1)
    assert(ee.entropyd(z) == 1)
    assert(ee.midd(x, y) == 0)
    assert(ee.midd(z, y) == 0)
    assert(ee.midd(z, x) == 0)
    assert(ee.cmidd(x, y, z) == 1)
    assert(ee.cmidd(z, y, x) == 1)
    assert(ee.cmidd(x, z, y) == 1)
...
def test_kldiv():
    ...
    res = [ee.kldiv(sample1, np.random.rand(200, 2)) for _ in range(1000)]
    assert(abs(np.mean(res)) < 0.2)
    sample1 += 3
    res2 = [ee.kldiv(sample1, np.random.rand(200, 2)) for _ in range(1000)]
